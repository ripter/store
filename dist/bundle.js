 ...
 */
var Store = function () {
  function Store(initalState, reducers, onChange) {
  ...
  _createClass(Store, [{
    key: 'action',
    value: function action(actionObj) {
      var onChange = this.onChange,
          _pendingActions = this._pendingActions,
          _state = this._state;
      ...
      while (_pendingActions.length > 0) {
        var nextActionObj = _pendingActions.pop();
    ...
    key: 'runReducers',
    value: function runReducers(state, actionObj) {
      var reducers = this.reducers,
          _pendingActions = this._pendingActions;

      var pushNextAction = function pushNextAction(nextActionObj) {
      ...
      return reducers.reduce(function (acc, reducer) {
        ...
        var newState = reducer(acc, actionObj, pushNextAction);
        // reducers *MUST* return the updated state. Check for errors.
        if (newState == null) {
          throw new Error('Store: Reducers must return a state object.\nStore: Got \'' + newState + '\' instead.');
        }
        return newState;
      }, state);
    }
    ...
    value: function get(path, defaultValue) {
      ...
      return (0, _lodash2.default)(_state, path, defaultValue);
    }
  }]);
